...
- Bundled darktable looks live in `styles/<id>/` with `style.dtstyle`, `thumbnail.jpg` and `style.json`. Regenerate thumbnails explicitly with `dev/style_preview <id>` after style changes; use the real engine and shared beach photograph.
