...
- Bundled darktable looks live in `presets/<id>/` with `preset.dtstyle`, `thumbnail.jpg` and `reference.json`. Regenerate thumbnails explicitly with `bin/preset_preview <id>` after style changes; use the real engine and shared beach photograph.
- Declare preset dependencies in `reference.json` using `style` and `assets` (`path`, `role`, optional `target`). Preserve these declarations when regenerating thumbnails. Unsupported or missing dependencies must not silently produce a different look.
