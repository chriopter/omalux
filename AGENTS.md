...
- `dev/tools/calibration/` fits bundled looks (`preset.dtstyle` + `look.cube`) to target renderings through `darktable-cli`; `dev/docs/calibration.md` has the data layout and recipe. Fits use tuning images only; holdout images are judged once by a finished candidate. Datasets and targets are not in the repository.

## Documentation layout

- Keep all project documentation in the `dev/docs/` directory. Current documentation lives directly there.
- Only the main `README.md` and this single `AGENTS.md` belong outside `dev/docs/`. Do not create component READMEs, additional documentation directories or additional agent instruction files.
...
3. Read `dev/docs/darktable-architecture.md` before engine changes and verify relevant findings against the installed/pinned source. Keep upstream changes explicit and minimal.
