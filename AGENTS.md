...
```
catalog/      what ships to darktable: styles/ (looks) and camera/ (camera presets, profiles)
omalux/       the Qt/QML application: UI, native adapter, launcher, smoke tests
development/  everything for working on it: launchers, docs/ and tools/
darktable/    unchanged upstream source, pinned by the submodule entry
...
- `omalux/` contains the Qt/QML UI, C/C++ native adapter and Python development launcher. The registered editing controls, crop, styles and JPEG/PNG export are connected to darktable; the History pane displays the current image’s native processing stack. See `development/docs/reference/controls.md` for the mappings and limitations. Do not describe planned functionality as released.
...
- `development/update` checks out the latest upstream stable release and its dependencies; it never stages, commits or pushes.
- `development/tools/calibration/` fits bundled looks (`style.dtstyle` + `look.cube`) to target renderings through `darktable-cli`; `development/docs/calibration.md` has the data layout and recipe. Fits use tuning images only; holdout images are judged once by a finished candidate. Datasets and targets are not in the repository.

## Documentation layout

- Keep all project documentation in `development/docs/`. Workflow documents (`setup.md`, `calibration.md`) sit directly there; `architecture/` holds engine internals and source audits, `reference/` holds controls, styles and assets. Keep `development/docs/README.md` as the entry point.
- Only the main `README.md` and this single `AGENTS.md` belong outside `development/docs/`. Do not create component READMEs, documentation directories elsewhere or additional agent instruction files.
...
3. Read `development/docs/architecture/darktable.md` before engine changes and verify relevant findings against the installed/pinned source. Keep upstream changes explicit and minimal.
