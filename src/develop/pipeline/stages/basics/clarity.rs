            ...
            for_each_restart_chunk(region.x, region.width, |anchor_x, end_x| {
                ...
                for global_x in anchor_x..end_x {
                    if global_x > anchor_x {
                        ...
                        sum_a -=
                            f64::from(sample(coefficient_a, width, height, outgoing_x, global_y));
                        sum_a +=
                            f64::from(sample(coefficient_a, width, height, incoming_x, global_y));
                        sum_b -=
                            f64::from(sample(coefficient_b, width, height, outgoing_x, global_y));
                        sum_b +=
                            f64::from(sample(coefficient_b, width, height, incoming_x, global_y));
                    }
                    if global_x >= region.x {
...
mod tests {
    ...
    fn adversarial_signed_guides_are_bit_exact_across_caller_partitions() {
        ...
        for square in [false, true] {
            let canonical = box_filter_tiled(&guide, width, height, square, width, height);
            for tile in [(1, 1), (3, 7), (15, 16), (17, 5), (31, 18)] {
                assert_eq!(
                    box_filter_tiled(&guide, width, height, square, tile.0, tile.1),
                    canonical,
        ...
        apply_tiled(&mut canonical, 100.0, width, height);
        for tile in [(1, 1), (3, 7), (15, 16), (17, 5), (31, 18)] {
            let mut partitioned = source.clone();
            apply_tiled(&mut partitioned, 100.0, tile.0, tile.1);
    ...
    fn rolling_sum_matches_the_direct_filter_with_tight_numeric_parity() {
        ...
        for square in [false, true] {
            ...
            for tile in [(1, 1), (7, 5), (16, 8), (41, 27)] {
                let rolling = box_filter_tiled(&guide, 41, 27, square, tile.0, tile.1);
    ...
    fn clarity_numeric_golden_pins_the_filter_constants_and_order() {
        ...
        apply_tiled(&mut image, 73.0, TILE_WIDTH, TILE_HEIGHT);
        let red_bits = image
            .pixels()
            .iter()
