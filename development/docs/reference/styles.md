...
Run `development/style_preview chromatic` from the repository root. It renders the shared beach image with the bundle's actual style through the Omalux/darktable adapter and writes a 384×256 JPEG plus `style.json` with `preview.source` and `preview.darktable_version`, preserving existing asset declarations. It requires the normal development dependencies and ImageMagick (`magick`). Use `development/style_preview --all` to regenerate every bundle in one persistent engine session. Each next style is applied after the preceding render completes. Regenerate explicitly after changing a style or LUT; startup only loads the stored thumbnail and does no extra preview rendering. A missing thumbnail displays “No preview” and does not prevent applying the style.
...
`style.dtstyle` and `thumbnail.jpg` are fixed filenames, so they are not repeated in JSON. The standard beach image is chosen by `development/style_preview`, not by each style. No file hashes are stored.
...
`development/style_preview` updates only the thumbnail, `preview.source` and `preview.darktable_version`, preserving asset declarations and other metadata. All bundled references were converted once to this format; archived v0 JSON remains unchanged.
...
This differs from `development/style_preview`, which deliberately regenerates catalogue thumbnails using the standard beach photograph. Neither operation is a visual calibration against v0.
