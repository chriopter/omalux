...
- **Clarity and noise reduction did not survive the one-time conversion.** The archived presets carry `clarity` and luminance/colour noise reduction; the converted styles had neither. The fit now seeds darktable's local contrast (`bilat`, detail = clarity/100) and `nlmeans` (luma and chroma = value/100) from the archive when it is present, and the slider search may move local contrast. A wrong local-contrast value cannot be judged with the cube fixed: it shifts tones that the cube had compensated, so the coupled trial is the only fair test.
