...
- PNG dimensions are checked before compressed ICC metadata is expanded. A structural pass records all color declarations, then only the declaration selected by normative priority is resolved; lower-priority iCCP payloads are never inflated. Selected compressed ICC, decompressed ICC, retained EXIF, and temporary inflate memory are cumulatively bounded. All formats select an audited `RasterRgba8` or `RasterRgba16` working-set profile before pixel allocation.
