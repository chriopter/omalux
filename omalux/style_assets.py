...
def read_manifest(bundle):
    path = bundle / 'style.json'
    if not path.exists():
        if (bundle / 'reference.json').exists():
            raise ValueError('Convert reference.json to style.json before loading this bundle')
        return {'version': 1}
...
def prepare_assets(catalogue, configs):
    ...
    for style in sorted(catalogue.rglob('style.dtstyle')):
