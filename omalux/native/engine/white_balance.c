...
static gboolean om_wb_matrices(dt_iop_module_t *module, double forward[4][3], double inverse[3][4]) {
    if (dt_image_is_raw(&module->dev->image_storage))
        return dt_colorspaces_conversion_matrices_xyz(module->dev->image_storage.adobe_XYZ_to_CAM,
                                                      module->dev->image_storage.d65_color_matrix, forward,
                                                      inverse);
    const double f[4][3] = {{3.2404542, -1.5371385, -0.4985314},
                            {-0.9692660, 1.8760108, 0.0415560},
                            {0.0556434, -0.2040259, 1.0572252},
                            {0, 0, 0}};
    const double i[3][4] = {{0.4124564, 0.3575761, 0.1804375, 0},
                            {0.2126729, 0.7151522, 0.0721750, 0},
                            {0.0193339, 0.1191920, 0.9503041, 0}};
    memcpy(forward, f, sizeof(f));
    memcpy(inverse, i, sizeof(i));
    return TRUE;
}
gboolean om_wb_read(dt_iop_module_t *module, const void *params, float *temperature, float *tint) {
    double f[4][3], i[3][4];
    if (!om_wb_matrices(module, f, i))
        return FALSE;
    double xyz[3] = {0};
    const float *coeffs = params;
    for (int a = 0; a < 3; ++a)
        for (int b = 0; b < 4; ++b)
            if (coeffs[b] > 0)
                xyz[a] += i[a][b] / coeffs[b];
