...
char *om_engine_history(void) {
  JsonArray *rows = json_array_new();
  int index = g_list_length(dev.history);
  for(GList *it = g_list_last(dev.history); it; it = it->prev) {
    ...
    json_object_set_boolean_member(row, "active", index <= dev.history_end);
    json_object_set_boolean_member(row, "current", index == dev.history_end);
  ...
  json_object_set_boolean_member(original,"current",dev.history_end==0);
...
int om_engine_history_select(int step) {
  if(!loaded || step < 0 || step > g_list_length(dev.history)) return 1;
  dt_dev_pop_history_items_ext(&dev, step);
  dt_dev_pixelpipe_rebuild(&dev);
  dt_dev_invalidate_all(&dev);
  return bind_controls();
}

char *om_engine_history_snapshot(const char *name) {
  return om_snapshot(name,"","*");
}
