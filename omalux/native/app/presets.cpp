...
std::vector<PresetFile> discoverPresets(const QString &directory) {
    ...
    while (files.hasNext()) {
        ...
        PresetFile preset{dir.relativeFilePath(entry.absoluteFilePath()),
        ...
        if (thumbnail.isFile())
            preset.previewUrl = QUrl::fromLocalFile(thumbnail.absoluteFilePath()).toString();
        QFile file(preset.path);
        if (!file.open(QIODevice::ReadOnly))
            preset.error = file.errorString();
        else {
                    ...
                    if (xml.name() == QStringLiteral("info")) {
                        while (xml.readNextStartElement()) {
                            if (xml.name() == QStringLiteral("name")) {
                                preset.name = xml.readElementText();
                                foundName = !preset.name.trimmed().isEmpty();
                            } else if (xml.name() == QStringLiteral("description"))
                                preset.description = xml.readElementText();
                            else if (xml.name() == QStringLiteral("iop_list")) {
                                if (!xml.readElementText().trimmed().isEmpty())
                                    preset.error = "Custom module order is not supported yet";
                            } else
                            ...
                            while (xml.readNextStartElement()) {
                                if (xml.name() == QStringLiteral("operation")) {
                                    if (xml.readElementText() == QStringLiteral("mask_manager"))
                                        preset.error = "Styles containing drawn masks are not supported yet";
                                } else
                                    xml.skipCurrentElement();
                            }
                        }
            ...
            while (!xml.atEnd())
                xml.readNext();
            if (xml.hasError())
                preset.error = xml.errorString();
            else if (!foundName || !foundStyle)
                preset.error = "Style name or settings missing";
        }
        if (assetErrors.contains(preset.id))
            preset.error = assetErrors.value(preset.id).toString();
    ...
    for (auto &preset : result) {
        ...
        for (const auto &other : result)
            if (other.name == preset.name)
                ++count;
        if (count > 1)
            preset.error = "Duplicate style name; give each style a unique name";
    }
